...
def load_dataset(name="dino"):
    if name == "dino":
        df = pd.read_csv("seed_datasets/apple-data.csv", header=None, names=['x','y'])
    elif name == "rando":
...
def generate_target_data(target, num_points=1000):
    if target == 'circle':
        # Circle parameters
        cx, cy, r = 54.26, 47.83, 30
        angles = np.linspace(0, 2 * np.pi, num_points)
        x = cx + r * np.cos(angles) + np.random.normal(0, 1, num_points)
        y = cy + r * np.sin(angles) + np.random.normal(0, 1, num_points)
    elif target == 'bullseye':
        # Bullseye parameters
        cx, cy = 54.26, 47.83
        r1, r2 = 18, 37
        num_points_each = num_points // 2
        angles1 = np.linspace(0, 2 * np.pi, num_points_each)
        x1 = cx + r1 * np.cos(angles1) + np.random.normal(0, 1, num_points_each)
        y1 = cy + r1 * np.sin(angles1) + np.random.normal(0, 1, num_points_each)
        angles2 = np.linspace(0, 2 * np.pi, num_points - num_points_each)
        x2 = cx + r2 * np.cos(angles2) + np.random.normal(0, 1, num_points - num_points_each)
        y2 = cy + r2 * np.sin(angles2) + np.random.normal(0, 1, num_points - num_points_each)
        x = np.concatenate((x1, x2))
        y = np.concatenate((y1, y2))
    elif target == 'dots':
        # Dots parameters
        centers = [[25, 20], [25, 50], [25, 80],
                   [50, 20], [50, 50], [50, 80],
                   [75, 20], [75, 50], [75, 80]]
        x = []
        y = []
        for center in centers:
            xi = center[0] + np.random.normal(0, 5, num_points // len(centers))
            yi = center[1] + np.random.normal(0, 5, num_points // len(centers))
            x.extend(xi)
            y.extend(yi)
    else:
        raise ValueError("Unsupported target shape for ideal data generation.")
...
def run_pattern(df, target, iters = 100000, num_frames=100, decimals=2, shake=0.2,
    ...
    # Generate ideal target data
    ideal_data = generate_target_data(target, num_points=len(df))

    # List to store error data
    error_data = []
    ...
    for i in looper(iters + 1, leave=True, ascii=True, desc=target + " pattern"):
        ...
        # Every 10,000 iterations, calculate and store error
        if (i + 1) % 10000 == 0:
            # Calculate mean error and standard deviation
            mean_error = np.mean(np.sqrt((r_good['x'] - ideal_data['x'])**2 + (r_good['y'] - ideal_data['y'])**2))
            std_error = np.std(np.sqrt((r_good['x'] - ideal_data['x'])**2 + (r_good['y'] - ideal_data['y'])**2))
            error_data.append({'Iteration': i + 1, 'Mean Error': mean_error, 'Std Error': std_error})
    ...
    # After all iterations, save error data to Excel
    error_df = pd.DataFrame(error_data)
    error_df.to_excel("results/compare-data.xlsx", index=False)
